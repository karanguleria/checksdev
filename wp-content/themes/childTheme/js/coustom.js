...
(function ($) {
    ...
    $(".choose-color").find('.choose-color-radio').click(function () {
        ...
        $(this).parent('.cfm-active-tab').parent().addClass('tc-active');
    ...
    jQuery(document).ready(function () {
        if (!(jQuery.type(getUrlParameter('tm_cart_item_key')) === "undefined")) {
            setTimeout(function () {
                jQuery('input.choose-color-radio[checked=checked]').prop('checked', false);
...
//var stickyBottom = $('.scroll_to_bottom');
     ...
     **/
    if (jQuery('input.cfm-custom-background').length > 0) {
...
function triggerLinesCheckbox(type) {
    if (type == 'software' && jQuery('#lines-checkbox').length) {
        jQuery('#lines-checkbox').show();
    } else {
        jQuery('#lines-checkbox').hide();
    }
}
